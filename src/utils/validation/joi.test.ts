import Joi, { SchemaLike, ValidationResult } from './joi';
import { BigNumber } from '@waves/data-entities';

const validate = <T>(s: SchemaLike) => (v: T | null) =>
  Joi.validate(v, s, { convert: false });
const assertPass = <T>(x: ValidationResult<T | null>) =>
  expect(x.error).toBeNull();
const assertError = <T>(x: ValidationResult<T | null>) =>
